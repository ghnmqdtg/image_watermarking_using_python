...
def encoded(watermark: np.array, target: np.array, K: int, save_pic: bool = False) -> np.array:
    """
    ...
    K: int
        Indicates how many last K bits to be encoded into
    save_pic: bool
...
def decode(target: np.array, K: int, save_pic: bool = False) -> np.array:
    """
    Decode a watermark from a encoded image
    ...
    target: np.array
        The np.array object of the target image
    K: int
        Indicates how many last K bits were the watermark encoded
    save_pic: bool
        Save the image or not
    """
...
def main():
    ...
    for bit in tqdm(encode_range, desc='Encode & Decode the watermark', bar_format='{l_bar}{bar:40}{r_bar}{bar:-10b}'):
        ...
        target_encoded = encoded(watermark_arr, target_arr, bit, True)
